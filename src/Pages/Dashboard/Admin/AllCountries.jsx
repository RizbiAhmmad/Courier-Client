...
const AllCountries = () => {
  ...
  const currentData =
    isDemo && localCountries.length ? localCountries : countries;
    ...
    try {
      if (isDemo) {
        const updated = currentData.map((item) =>
          item._id === selectedCountry._id
            ? { ...item, name: formData.name, status: formData.status }
            : item
        );
        setLocalCountries(updated);
        setIsModalOpen(false);
        Swal.fire("Demo Mode", "Country updated temporarily!", "info");
        return;
      }

      await axiosPublic.put(`/countries/${selectedCountry._id}`, formData);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        if (isDemo) {
          ...
          Swal.fire("Demo Mode", "Country deleted temporarily!", "info");
          return;
        }

        const res = await axiosPublic.delete(`/countries/${id}`);
  ...
  return (
    <div className="max-w-2xl p-6 mx-auto">
      ...
      <div className="flex justify-end mb-4">
        <button
          onClick={() => navigate("/dashboard/addCountry")}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md relative">
            ...
            <h3 className="mb-4 text-xl font-semibold">Edit Country</h3>
