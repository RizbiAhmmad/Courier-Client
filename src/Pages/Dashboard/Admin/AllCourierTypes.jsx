...
const AllCourierTypes = () => {
  ...
  const { data: courierTypes = [], refetch } = useQuery({
  ...
  const [localCourierTypes, setLocalCourierTypes] = useState([]);
  const currentData =
    isDemo && localCourierTypes.length ? localCourierTypes : courierTypes;
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (isDemo) {
        ...
        setLocalCourierTypes(updated);
        setIsModalOpen(false);
        Swal.fire("Demo Mode", "Courier Type updated temporarily!", "info");
        return;
      }

      await axiosPublic.put(`/courierTypes/${selectedItem._id}`, {
        name: formData.name,
        status: formData.status,
      });

      Swal.fire("Updated!", "Courier Type has been updated.", "success");
      setIsModalOpen(false);
      refetch();
    } catch (err) {
      Swal.fire("Error", "Failed to update Courier Type", "error");
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This Courier Type will be deleted!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        if (isDemo) {
          const filtered = currentData.filter((item) => item._id !== id);
          setLocalCourierTypes(filtered);
          Swal.fire("Demo Mode", "Courier Type deleted temporarily!", "info");
          return;
        }

        const res = await axiosPublic.delete(`/courierTypes/${id}`);
        if (res.data.deletedCount > 0) {
          refetch();
          Swal.fire("Deleted!", "Courier Type removed.", "success");
        }
      }
    });
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md relative">
            ...
            <h3 className="mb-4 text-xl font-semibold">Edit Courier Type</h3>
...
export default AllCourierTypes;
